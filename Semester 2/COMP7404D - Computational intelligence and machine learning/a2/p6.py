...
def expecti_max_multiple_ghosts(problem, k):
    """
    Pacman agent using Expectimax search against multiple random ghosts.
    """
    seed = problem['seed']
    if seed != -1:
    ...
    while True:
        step += 1
        # Pacman chooses the best move using Expectimax
        direction = expectimax_decision(pacman_position, ghost_positions, food_positions, wall_positions, depth=k)
        ...
        # Each ghost moves stochastically
...
def expectimax_decision(pacman_position, ghost_positions, food_positions, wall_positions, depth):
    """ Expectimax search to determine the best move for Pacman. """
    def expectimax(state, depth, agent_index):
        ...
        if agent_index == 0:  # Pacman (Max node)
            ...
            for action in DIRECTIONS:
                ...
                value = expectimax({'pacman': new_position, 'ghosts': state['ghosts'], 'food': state['food']}, depth - 1, 1)
                if value > best_value:
                    best_value = value
                    best_action = action
            return best_action if depth == k else best_value
            ...
            for action in valid_actions:
                ...
                values.append(expectimax({'pacman': state['pacman'], 'ghosts': new_ghosts, 'food': state['food']}, next_depth, next_agent))
            
            return sum(values) / len(values) if values else 0  # Expected value
    
    state = {'pacman': pacman_position, 'ghosts': ghost_positions, 'food': food_positions}
    return expectimax(state, depth, 0)
