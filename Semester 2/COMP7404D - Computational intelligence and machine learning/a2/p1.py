...
def random_play_single_ghost(problem):
    ...
    pacman_position, ghost_position, food_positions, board_size = extract_board_state(layout)
    score = 0
    step = 0
    result_str = ''
    result_str += 'seed: {}\n'.format(seed)
    result_str += '0\n'
    result_str += reconstruct_board_state(pacman_position, ghost_position, food_positions, board_size)
    ...
    while True:
        ...
        for direction in DIRECTIONS:
            i, j = move(pacman_position, direction, board_size)
            if layout[i][j] != '%':
                available_directions.append(direction)
        direction = random.choice(available_directions)
        p_moving_str = f'{step}: P moving {direction}'
        pacman_position = move(pacman_position, direction, board_size)
        board_str = reconstruct_board_state(pacman_position, ghost_position, food_positions, board_size)
        ...
        available_directions = []
        for direction in DIRECTIONS:
            i, j = move(ghost_position, direction, board_size)
            if layout[i][j] != '%':
                available_directions.append(direction)
        direction = random.choice(available_directions)
        w_moving_str = f'{step}: W moving {direction}'
        ghost_position = move(ghost_position, direction, board_size)
        board_str = reconstruct_board_state(pacman_position, ghost_position, food_positions, board_size)
...
def move(position, direction, board_size):
    i, j = position
    if direction == 'E':
        j += 1
    elif direction == 'W':
...
def extract_board_state(layout):
    ...
    return pacman_position, ghost_position, food_positions, board_size

def reconstruct_board_state(pacman_position, ghost_position, food_positions, board_size):
    """
    '%': Wall
    'W': Ghost
    'P': Pacman
