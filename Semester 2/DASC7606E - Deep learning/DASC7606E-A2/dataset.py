...
def align_labels_with_tokens(
    ...
    for word_id in word_ids:
        if word_id is None:
            # Special token, assign -100
            aligned_labels.append(-100)
        elif word_id != previous_word_id:
            # New word, assign the label as is
            aligned_labels.append(labels[word_id])
        else:
            # Same word, assign the label with I- prefix if necessary
            label = labels[word_id]
            if label % 2 == 1:  # If the label is B- (odd number), convert to I- (even number)
                label += 1
